  ...
  <div class="zp-content">
    ...
    <div class="zhuanpan-root">
      ...
      <swiper auto height="30px" direction="vertical" :interval=2000 class="laba text-scroll" :show-dots="false">
        <swiper-item v-for="msg in fakeAwards" :key="msg">
          <p>{{msg}}</p>
        </swiper-item>
      </swiper>
...
.value-root>div {
  ...
  // flex: 1;
  width: 1.96rem;
  height: .58rem; // margin-right: .42rem;
  background-size: 100% 100%;
  display: flex;
  justify-content: center;
  align-content: center;
...
.zhuanpan {
  ...
  height: 7.6rem;
  overflow: hidden;
  box-sizing: border-box; // padding-top: .16rem;
  display: flex;
  justify-content: center;
  align-items: center;
  ...
  .btn {
    width: 2rem;
    height: 2.14rem;
    background: url(../assets/zhuanpan/button-start1.png) no-repeat;
    background-size: contain;
  }
  .ball {
    position: absolute;
  ...
  .ball2 {
    transform: rotate(-90deg) translateY(-1.8rem); // p{
...
.laba {
  ...
  height: .58rem;
  margin-left: auto;
  margin-right: auto; // bottom:.05rem;
  bottom: 0;
